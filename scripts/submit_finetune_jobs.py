...
if experiment_group == "dataset_comparison":
    ...
    for dataset in datasets:
        ...
        if dataset == "combined":
            ...
            combining_bash_command = (
                "cat "
                + " ".join([f"/data/{d}/{d}_data.jsonl" for d in combining_datasets])
                + " > /output/combined_data.jsonl"
            )
            d["tasks"][0]["arguments"][0] = (
                combining_bash_command + " && " + d["tasks"][0]["arguments"][0]
            )
        ...
        d["tasks"][0]["envVars"].append({"name": "WANDB_API_KEY", "value": wandb_api_key})
