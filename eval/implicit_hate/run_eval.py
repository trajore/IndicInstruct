...
def gen_prompt(dev_data, k=-1):
    prompt = "Is the following statement hateful or neutral? The possible choices for the label are: 'hate' and 'neutral'.\n\n"
    if k > 0:
        exemplars = dev_data.select(range(k))
        for example in exemplars:
            prompt += format_example(text=example["text"], label=example["label"])
    return prompt
...
def main(args):
    ...
    dataset = load_dataset("dipteshkanojia/implicit_hate")
    ...
    for i, example in enumerate(test_data):
        k = args.ntrain
        prompt_end = format_example(example["text"])
