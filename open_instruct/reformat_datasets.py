...
def convert_unnatural_instructions_data(data_dir, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    instance_cnt = 0
    with open(os.path.join(data_dir, "core_data.jsonl"), "r") as fin, open(
...
def convert_baize_data(data_dir, output_dir):
    ...
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            ...
            fout.write(
                json.dumps({"dataset": "baize", "id": f"baize_{idx}", "messages": messages}) + "\n"
            )
    ...
    with open(output_path, "w") as fout:
        ...
        for _, conversation in enumerate(conversations):
            ...
            for sequence in valid_sequences:
                fout.write(
                    json.dumps(
                        {"dataset": "oasst1", "id": f"oasst1_{example_cnt}", "messages": sequence}
                    )
                    + "\n"
                )
                example_cnt += 1
    ...
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            ...
            for i in range(0, len(example["conversations"]), 2):
                ...
                messages.append(
                    {"role": "assistant", "content": example["conversations"][i + 1]["value"]}
                )
            fout.write(
                json.dumps(
                    {
...
if __name__ == "__main__":
    ...
    for subfolder in subfolders:
        if subfolder not in supported_datasets:
            print(
                f"Warning: {subfolder} in the raw data folder is not a supported dataset. We will skip it."
            )
        else:
    ...
    for subfolder in valid_subfolders:
        ...
        globals()[f"convert_{subfolder}_data"](
            os.path.join(args.raw_data_dir, subfolder), os.path.join(args.output_dir, subfolder)
        )
